...
class Resize(object):
    ...
    def __init__(self, size, interpolation='nearest'):
        assert isinstance(size, int) or \
               (isinstance(size[0], int) and len(size) == 2)

        if isinstance(size,float):
            size = int((size+0.99999999)//1)
        self.size = size
    ...
    def __call__(self, img):
        ...
        #         size = (img.shape[0]/min(img.shape)*self.size//1,img.shape[1]/min(img.shape)*self.size//1)
        #         ret = cv2.resize(img, size, interpolation=self.interpolation)
        #     else:
        #         ret = cv2.resize(img, self.size, interpolation=self.interpolation)
        # except:
        #     print('_______________---')
        #     print(img.size,self.size,self.interpolation)
        # return ret
        return numpy.array(Image.fromarray(img).resize(self.size,self.interpolation))
        if img.ndim == 3:
